...
export default function CoachDashboardPage() {
  ...
  async function loadData() {
    ...
    const athleteDataList: AthleteData[] = members.map((m: any) => {
      ...
      if (latestCheckin) {
        const wDate = new Date(latestCheckin.date + 'T12:00:00')
        checkinDaysAgo = Math.floor((new Date().getTime() - wDate.getTime()) / (1000 * 60 * 60 * 24))
      }
  ...
  const stats = useMemo(() => {
    ...
    return { total, loggedToday, avgCompliance, topCompliance, topFuelScore, redFlagCount: redFlags.length, wellnessAlertCount: wellnessAlerts.length, avgFuelScore, lockedIn, onTrack, dialItIn, redFlag, noCheckin }
              ...
              {showMoreMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-xl shadow-xl z-40 overflow-hidden">
                    ...
                    <button
                      onClick={() => { router.push('/coach/scheduled-messages'); setShowMoreMenu(false) }}
                      className="w-full px-4 py-3 text-left text-sm text-slate-300 hover:bg-slate-700 transition-colors flex items-center gap-3"
                    >
                      <svg className="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      Scheduled Messages
                    </button>
                    {!isAdmin && (
                      <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5">
            <p className="text-slate-400 text-[11px] font-semibold uppercase tracking-widest">Avg Fuel Score</p>
            <p className="text-4xl font-bold mt-2">
              {stats.avgFuelScore !== null ? (
                <span className={getWellnessColor(stats.avgFuelScore)}>{stats.avgFuelScore}%</span>
              ) : (
