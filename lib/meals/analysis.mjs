...
const FRIENDLY_DETAIL_LABELS = {
  ...
  package_label: 'the nutrition-label values for the branded or packaged food',
...
function unresolvedModelDetails(values, clarification = '') {
  const answer = String(clarification || '').trim().toLowerCase()
  return values.filter(detail => {
    if (!answer) return true
    if (detail === 'portion_unit') return !hasPortionUnit(answer)
    if (detail === 'serving_size') {
      return !hasPortionUnit(answer) && !/\b(?:label|package)\b/i.test(answer)
    }
    if (detail === 'package_label') {
      return !/\b(?:label|package|nutrition|calories|protein|carbs?|fat)\b/i.test(answer)
    }
    if (detail === 'raw_or_cooked') return !includesAny(answer, PREPARATION_WORDS)
    if (detail === 'meal_details') return answer.length < 3
    return true
  })
}
...
export function buildClarifyingQuestion(missingDetails) {
  ...
  if (missing.length === 1) {
    switch (missing[0]) {
      ...
      case 'package_label':
        return 'What does the nutrition label list for the amount you had, or would you rather enter the label macros directly?'
      case 'raw_or_cooked':
...
export function normalizeMealAnalysis(rawResult, context = {}) {
  ...
  const modelMissingCandidates = Array.isArray(rawMissingDetails)
    ? rawMissingDetails.map(value => String(value).trim()).filter(Boolean)
    : []
  const modelMissing = unresolvedModelDetails(modelMissingCandidates, context.clarification)
