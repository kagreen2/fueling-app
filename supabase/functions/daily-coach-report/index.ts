  ...
  const actionItemsSection = actionItems.length > 0 ? `
    <div style="background-color: #1e293b; border-radius: 12px; overflow: hidden; margin-bottom: 24px; border: 1px solid #334155;">
      <div style="padding: 14px 16px; background-color: #1a2332; border-bottom: 1px solid #334155;">
        <h2 style="color: ${orgPrimaryColor}; font-size: 15px; margin: 0;">📋 Action Items</h2>
      </div>
      <div style="padding: 12px 16px;">
        ${actionItems.join('\n')}
      </div>
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 24px;">
    ...
    <div style="text-align: center; padding: 24px 0; border-top: 1px solid #334155;">
      <p style="color: #64748b; font-size: 13px; margin: 0;">
        This report was generated by <span style="color: ${orgPrimaryColor};">${orgName}</span>
      </p>
      <p style="color: #475569; font-size: 12px; margin: 8px 0 0;">
        <a href="${orgDashboardUrl}" style="color: ${orgPrimaryColor}; text-decoration: none;">View Full Dashboard →</a>
      </p>
    </div>
...
Deno.serve(async (req) => {
  try {
    ...
    for (const coach of coaches) {
      try {
        // 2. Get coach's organization for branding
        let orgName = 'Fuel Different'
        let orgPrimaryColor = '#a78bfa'
        let orgEmailFrom = 'Fuel Different <reports@fueldifferent.app>'
        let orgDashboardUrl = 'https://www.fueldifferent.app/coach/dashboard'

        // Try profile.org_id first
        ...
        if (coachProfile?.org_id) {
          const { data: orgData } = await supabase
            .from('organizations')
            .select('name, primary_color, custom_email_from, slug')
            .eq('id', coachProfile.org_id)
            .single()
          if (orgData) {
            orgName = orgData.name
            orgPrimaryColor = orgData.primary_color || '#a78bfa'
            if (orgData.custom_email_from) {
              orgEmailFrom = `${orgData.custom_email_from} <reports@fueldifferent.app>`
            }
            if (orgData.slug && orgData.slug !== 'default') {
              orgDashboardUrl = `https://${orgData.slug}.fueldifferent.com/coach/dashboard`
            }
          }
        } else {
          // Fallback: check if any of the coach's teams have an org_id
        ...
        // 2b. Get athletes assigned to this coach
